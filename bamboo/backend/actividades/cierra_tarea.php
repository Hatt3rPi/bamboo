...
if ($_POST["accion"]=='cerrar_tarea'){
  ...
  db_query($link, $query);
  db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Cierra tarea', '".str_replace("'","**",$query)."','tarea',".$id.", '".$_SERVER['PHP_SELF']."')");
}
elseif($_POST["accion"]=='cerrar_tarea_recurrente'){
  ...
  db_query($link, $query);
  db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Cierra tarea recurrente', '".str_replace("'","**",$query)."','tarea',".$id.", '".$_SERVER['PHP_SELF']."')");
}
else {
  $query='update tareas set estado="Eliminado", fecha_completada=current_date WHERE id='.$id.';';
db_query($link, $query);
db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Elimina tarea', '".str_replace("'","**",$query)."','tarea',".$id.", '".$_SERVER['PHP_SELF']."')");
}
