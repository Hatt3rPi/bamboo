...
switch ($_POST["accion"]) {
  case 'eliminar_poliza':
    ...
    $mensaje='Póliza eliminada correctamente';
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Elimina póliza', '".str_replace("'","**",$query)."','poliza','".$busqueda."', '".$_SERVER['PHP_SELF']."')");
      break;
  case 'cancelar_poliza':
    ...
    $busqueda=estandariza_info($_POST["numero_poliza"]);
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Cancela póliza', '".str_replace("'","**",$query)."','poliza','".$busqueda."', '".$_SERVER['PHP_SELF']."')");
      $query_2='select id, numero_poliza from polizas_2 where id='.$busqueda.';';
      $resultado = db_query($link, $query_2);
  ...
  case 'reactivar_poliza':
    ...
    $busqueda=estandariza_info($_POST["numero_poliza"]);
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Reactiva póliza', '".str_replace("'","**",$query)."','poliza','".$busqueda."', '".$_SERVER['PHP_SELF']."')");
      $query_2='select id, numero_poliza from polizas_2 where id='.$busqueda.';';
      $resultado = db_query($link, $query_2);
      while ($fila = db_fetch_object($resultado))
      {
  ...
  case 'anular_poliza':
    ...
    $busqueda=estandariza_info($_POST["numero_poliza"]);
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Anula póliza', '".str_replace("'","**",$query)."','poliza','".$busqueda."', '".$_SERVER['PHP_SELF']."')");
          $query_2='select id, numero_poliza from polizas_2 where id='.$busqueda.';';
      $resultado = db_query($link, $query_2);
      while ($fila = db_fetch_object($resultado))
      {
  ...
  case 'rechazar_propuesta':
    ...
    $mensaje='Propuesta Póliza rechazada correctamente';
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Rechaza propuesta póliza', '".str_replace("'","**",$query)."','propuesta_poliza','".$nro_propuesta."', '".$_SERVER['PHP_SELF']."')");
      break;
  case 'envio_propuesta':
    ...
    $mensaje='Propuesta Póliza generada correctamente';
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Envía propuesta póliza', '".str_replace("'","**",$query)."','propuesta_poliza','".$nro_propuesta."', '".$_SERVER['PHP_SELF']."')");
      break;
  ...
  case 'actualiza_propuesta': 
    $busqueda=$nro_propuesta;
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Actualiza propuesta póliza', '".str_replace("'","**",$query)."','propuesta_poliza','".$nro_propuesta."', '".$_SERVER['PHP_SELF']."')");
      foreach ($_POST["rutaseg"] as $key => $valor) 
      {
        ...
        db_query($link, $query);
        db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Actualiza ítem', '".str_replace("'","**",$query)."','Ítems',CONCAT('".$nro_propuesta."','[','".(intval($key)+1)."', ']'), '".$_SERVER['PHP_SELF']."')");
      }
      $query="delete from items where numero_propuesta='".$nro_propuesta."' and numero_item>".$contador_items.";";
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Corrige cantidad de ítems', '".str_replace("'","**",$query)."','Ìtems','".$nro_propuesta."', '".$_SERVER['PHP_SELF']."')");
      break;
  ...
  case 'crear_propuesta':
    ...
    $poliza_renovada= estandariza_info($_POST["poliza_renovada"]);
        ...
        db_query($link, $query);
        db_query($link, 'update propuesta_polizas set numero_propuesta=CONCAT(\'P\', LPAD(id,6,0)) where token=\'' . $token . '\';');
        $resultado = db_query($link, 'select id, numero_propuesta from propuesta_polizas where token=\'' . $token . '\';');
        ...
        db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Creación propuesta póliza', '".str_replace("'","**",$query)."','propuesta_poliza', '".$nro_propuesta."' , '".$_SERVER['PHP_SELF']."')");
    // Incorporar ítems
      ...
      foreach ($_POST["rutaseg"] as $key => $valor) 
      {
        ...
        db_query($link, $query_items);
        db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Agrega ítems', '".str_replace("'","**",$query_items)."','Ítems', CONCAT('".$nro_propuesta."','[','".(intval($key)+1)."', ']') , '".$_SERVER['PHP_SELF']."')");
      }
        ...
        $query= "update polizas_2 set estado_renovacion='Renovado', comentarios_int=concat(comentarios_int,'; ', DATE_FORMAT(CURRENT_DATE,'%d/%m/%Y'), ' es renovada por propuesta póliza ".$nro_propuesta."') where numero_poliza='".$poliza_renovada."';";
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Renovación póliza - antigua', '".str_replace("'","**",$query)."','poliza','".$poliza_renovada."', '".$_SERVER['PHP_SELF']."')");
        $query= "update propuesta_polizas set poliza_renovada='".$poliza_renovada."' , comentarios_int=concat(comentarios_int,'; ', DATE_FORMAT(CURRENT_DATE,'%d/%m/%Y'), ' renueva póliza ".$poliza_renovada."') where id='".$id_propuesta."';";
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Renovación póliza - nueva', '".str_replace("'","**",$query)."','propuesta poliza','".$id_propuesta."', '".$_SERVER['PHP_SELF']."')");
        }
  ...
  case 'crear_poliza':
    ...
    $busqueda=estandariza_info($_POST["nro_poliza"]);
        ...
        $listado='/bamboo/listado_polizas.php';
            ...
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Actualiza ítem', '".str_replace("'","'",$query)."','Ítems',CONCAT('".$nro_propuesta."','[all]'), '".$_SERVER['PHP_SELF']."')");
      ...
      $query="INSERT INTO items(numero_propuesta, numero_poliza,  numero_item, rut_asegurado, dv_asegurado, materia_asegurada, patente_ubicacion, cobertura, deducible, tasa_afecta, tasa_exenta, prima_afecta, prima_exenta, prima_neta, prima_bruta_anual, monto_asegurado, venc_gtia) VALUES ('".$nro_propuesta."','".$nro_poliza."',  '".(intval($key)+1)."','".$rut_aseg."','".$dv_aseg."','".$materia."','".$detalle_materia."','".$cobertura."','".$deducible."','".$tasa_afecta."','".$tasa_exenta."','".$prima_afecta."','".$prima_exenta."','".$prima_neta."','".$prima_bruta."','".$monto_aseg."','".$venc_gtia."') 
        ON DUPLICATE KEY UPDATE rut_asegurado='".$rut_aseg."',dv_asegurado='".$dv_aseg."',materia_asegurada='".$materia."',patente_ubicacion='".$detalle_materia."',cobertura='".$cobertura."',deducible='".$deducible."', tasa_afecta='".$tasa_afecta."', tasa_exenta='".$tasa_exenta."', prima_afecta='".$prima_afecta."', prima_exenta='".$prima_exenta."', prima_neta='".$prima_neta."', prima_bruta_anual='".$prima_bruta."', monto_asegurado='".$monto_aseg."', venc_gtia='".$venc_gtia."' , fecha_ultima_modificacion=CURRENT_TIMESTAMP;";
          ...
          db_query($link, $query);
          db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Aprueba propuesta póliza', '".str_replace("'","**",$query)."','propuesta_poliza','".$nro_propuesta."', '".$_SERVER['PHP_SELF']."')");
        ...
        db_query($link, $query);
        $resultado = db_query($link, 'select id, numero_poliza from polizas_2 where token=\'' . $token . '\';');
        while ($fila = db_fetch_object($resultado))
        {
            // printf ("%s (%s)\n", $fila->id);
            $id_poliza = $fila->id;
        ...
        db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Creación póliza', '".str_replace("'","**",$query)."','poliza', '".$id_poliza."' , '".$_SERVER['PHP_SELF']."')");
    // corrige ítems
    $query="delete from items where numero_propuesta='".$nro_propuesta."' and numero_item>".$contador_items.";";
    db_query($link, $query);
    db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Corrige cantidad de ítems', '".str_replace("'","**",$query)."','Ìtems','".$nro_propuesta."', '".$_SERVER['PHP_SELF']."')");
    if ($poliza_renovada_desde_propuesta) {
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Actualiza estado de la póliza renovada', '".str_replace("'","**",$query)."','poliza', '".$poliza_renovada_desde_propuesta."', '".$_SERVER['PHP_SELF']."')");
  }
      ...
      $busqueda=estandariza_info($_POST["nro_poliza"]);
        ...
        $query="INSERT INTO items(numero_propuesta, numero_poliza,  numero_item, rut_asegurado, dv_asegurado, materia_asegurada, patente_ubicacion, cobertura, deducible, tasa_afecta, tasa_exenta, prima_afecta, prima_exenta, prima_neta, prima_bruta_anual, monto_asegurado, venc_gtia) VALUES ('".$nro_propuesta."','".$nro_poliza."',  '".(intval($key)+1)."','".$rut_aseg."','".$dv_aseg."','".$materia."','".$detalle_materia."','".$cobertura."','".$deducible."','".$tasa_afecta."','".$tasa_exenta."','".$prima_afecta."','".$prima_exenta."','".$prima_neta."','".$prima_bruta."','".$monto_aseg."','".$venc_gtia."') 
          ON DUPLICATE KEY UPDATE rut_asegurado='".$rut_aseg."',dv_asegurado='".$dv_aseg."',materia_asegurada='".$materia."',patente_ubicacion='".$detalle_materia."',cobertura='".$cobertura."',deducible='".$deducible."', tasa_afecta='".$tasa_afecta."', tasa_exenta='".$tasa_exenta."', prima_afecta='".$prima_afecta."', prima_exenta='".$prima_exenta."', prima_neta='".$prima_neta."', prima_bruta_anual='".$prima_bruta."', monto_asegurado='".$monto_aseg."', venc_gtia='".$venc_gtia."' , fecha_ultima_modificacion=CURRENT_TIMESTAMP;";
                ...
                //$query='UPDATE items SET numero_poliza=\'' . $nro_poliza . '\', rut_asegurado=\'' . $rut_aseg . '\',dv_asegurado=\'' . $dv_aseg . '\',materia_asegurada=\'' . $materia . '\',patente_ubicacion=\'' . $detalle_materia . '\',cobertura=\'' . $cobertura . '\',deducible=\'' . $deducible . '\', tasa_afecta=\'' . $tasa_afecta . '\', tasa_exenta=\'' . $tasa_exenta . '\', prima_afecta=\'' . $prima_afecta . '\', prima_exenta=\'' . $prima_exenta . '\', prima_neta=\'' . $prima_neta . '\', prima_bruta_anual=\'' . $prima_bruta . '\', monto_asegurado=\'' . $monto_aseg . '\', venc_gtia=\'' . $venc_gtia . '\', fecha_ultima_modificacion=CURRENT_TIMESTAMP WHERE numero_propuesta=\'' . $nro_propuesta . '\' and numero_item=\'' . $numero_item . '\';';
                db_query($link, $query);
                db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Crea ítem', '".str_replace("'","**",$query)."','Ítems',CONCAT('".$nro_propuesta."','[','".(intval($key)+1)."', ']'), '".$_SERVER['PHP_SELF']."')");
              }
          //crea token
          $largo = 6;
          $token = bin2hex(random_bytes($largo));
        ...
        $query= "update polizas_2 set estado_renovacion='Renovado', estado='Renovado', comentarios_int=concat(comentarios_int,'; ', DATE_FORMAT(CURRENT_DATE,'%d/%m/%Y'), ' es renovada por póliza ".$nro_propuesta."') where numero_poliza='".$poliza_renovada."';";
            ...
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Renovación póliza - nueva', '".str_replace("'","**",$query)."','poliza','".$id_poliza."', '".$_SERVER['PHP_SELF']."')");
  ...
  case 'modifica_poliza':
    $busqueda=estandariza_info($_POST["nro_poliza"]);
      ...
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Actualiza Póliza', '".str_replace("'","**",$query)."','poliza','".$nro_poliza."', '".$_SERVER['PHP_SELF']."')");
      foreach ($_POST["rutaseg"] as $key => $valor) 
      {
        $rut_completo_aseg = str_replace("-", "", estandariza_info($_POST["rutaseg"][$key]));
        $rut_aseg=estandariza_info(substr($rut_completo_aseg, 0, strlen($rut_completo_aseg)-1));
        ...
        db_query($link, $query);
        db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Actualiza ítem', '".str_replace("'","**",$query)."','Ítems',CONCAT('".$nro_poliza."','[','".(intval($key)+1)."', ']'), '".$_SERVER['PHP_SELF']."')");
      }
      $query="delete from items where numero_poliza='".$nro_poliza."' and numero_item>".$contador_items.";";
      db_query($link, $query);
      db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Corrige cantidad de ítems', '".str_replace("'","**",$query)."','Ìtems','".$nro_poliza."', '".$_SERVER['PHP_SELF']."')");
      break;
    //echo $query;
}
