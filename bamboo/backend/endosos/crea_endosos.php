...
if ($_SERVER[ "REQUEST_METHOD" ] == "POST")
    ...
    switch ($_POST["accion"]){
        case 'crea_propuesta_endoso_manual':
            ...
            db_query($link, $query);
            db_query($link, 'update propuesta_endosos set numero_propuesta_endoso=CONCAT(\'E\', LPAD(id,6,0)) where token=\'' . $token . '\';');
            $resultado = db_query($link, 'select id, numero_propuesta_endoso from propuesta_endosos where token=\'' . $token . '\';');
            ...
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Creación propuesta endoso', '".str_replace("'","**",$query)."','propuesta_endoso', '".$id_poliza."' , '".$_SERVER['PHP_SELF']."')");
        ...
        case 'actualiza_propuesta':
            ...
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Modificación propuesta endoso', '".str_replace("'","**",$query)."','propuesta_endoso', '".$_POST["numero_propuesta_endoso"]."' , '".$_SERVER['PHP_SELF']."')");
            $busqueda=$numero_propuesta_endoso;
            break;
        case 'actualiza_endoso':
            ...
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Modificación endoso', '".str_replace("'","**",$query)."','endoso', '".$id_endoso."' , '".$_SERVER['PHP_SELF']."')");
            $busqueda=$numero_propuesta_endoso;
            break;
        case 'crear_endoso':
            $numero_propuesta_endoso=cambia_puntos_por_coma(estandariza_info($_POST["numero_propuesta_endoso"]));
            ...
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Creación endoso', '".str_replace("'","**",$query)."','endoso', '".$numero_endoso."' , '".$_SERVER['PHP_SELF']."')");
            $busqueda=$numero_propuesta_endoso;
            
            $query= "update propuesta_endosos set estado='Emitido'  where id='".$id_propuesta_endoso."'";
            db_query($link, $query);
            db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Aprueba propuesta endoso', '".str_replace("'","**",$query)."','propuesta_endoso','".$_POST["numero_propuesta_endoso"]."', '".$_SERVER['PHP_SELF']."')");
            ...
            case 'crea_propuesta_endoso_web':
                ...
                db_query($link, $query);
                db_query($link, 'update endosos set numero_propuesta_endoso=CONCAT(\'E2\', LPAD(id,5,0)) where token=\'' . $token . '\';');
                $resultado = db_query($link, 'select id, numero_propuesta_endoso from endosos where token=\'' . $token . '\';');
                while ($fila = db_fetch_object($resultado))
                {
                    // printf ("%s (%s)\n", $fila->id);
                    $id= $fila->id;
                ...
                $busqueda=$numero_propuesta_endoso;
                db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Creación endoso web', '".str_replace("'","**",$query)."','endoso', '".$id."' , '".$_SERVER['PHP_SELF']."')");
                break;
        case 'rechazar_propuesta':
            ...
            $mensaje='Propuesta Endoso rechazada correctamente';
                ...
                db_query($link, $query);
                db_query($link, "select trazabilidad('".$_SESSION["username"]."', 'Rechaza propuesta endoso', '".str_replace("'","**",$query)."','propuesta_endoso','".$id_propuesta_endoso."', '".$_SERVER['PHP_SELF']."')");
                break;
    }
